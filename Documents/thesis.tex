...
Przede wyszystkim dlatego, że pliki z rozszerzeniem .dae są łatwe zarówno do stworzenia, jak i do odczytania, ze względu na swoją strukturę. W dodatku, są dostępne opracowane i przetestowane narzędzia do parsowania plików w tym formacie. 
...
Jednak niepoprawna struktura pliku Collada przełożyła się również na współrzędne użytych materiałów. Tych danych już niestety nie udało się naprawić w dostępnych programach graficznych. Dlatego wczytaną siatkę modelu pokolorowano w jeden podstawowy kolor (niebieski). Na rysunku \ref{rys27} widać kształt detektora, lecz z braku możliwości wglądu w warstwy wewnętrzne i braku poprawnych materiałów, obraz ten wygląda prymitywnie.

\begin{figure}[H]
		\centering
 		\includegraphics[width=8.0cm]{myGeom.png}
 		\captionsetup{font={up, footnotesize}}
